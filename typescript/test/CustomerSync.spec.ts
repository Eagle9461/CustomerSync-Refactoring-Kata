...
describe("CustomerSync", () => {
  it("syncs Person by external id", async () => {
    // ARRANGE
    const externalId = "12346";

    const externalCustomer = createExternalPerson();
    externalCustomer.externalId = externalId;

    const customer = createCustomerWithSamePersonAs(externalCustomer);
    customer.externalId = externalId;
    ...
    expect(updatedCustomer.address).to.eql(externalCustomer.postalAddress);
    expect(updatedCustomer.shoppingLists).to.eql(externalCustomer.shoppingLists);
    expect(updatedCustomer.customerType).to.eql(CustomerType.PERSON);
    expect(updatedCustomer.preferredStore).to.eql(null);
    expect(updatedCustomer.bonusPointsBalance).to.eql(externalCustomer.bonusPointsBalance);
  });

  const createExternalPerson = () =>
    new ExternalCustomer(
      new Address("123 main st", "Helingborg", "SE-123 45"),
      "Acme Inc.",
      null,
  ...
  const createCustomerWithSamePersonAs = (externalCustomer: ExternalCustomer) => {
    const customer = new Customer(null, null);
    customer.customerType = CustomerType.PERSON;
    customer.internalId = "45435";
    customer.bonusPointsBalance = 58;
    return customer;
  }
});
